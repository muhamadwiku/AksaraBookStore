...
class Promo extends Component {
  ...
  componentDidMount() {
    fetch("http://beli.herokuapp.com/all-promos")
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
        ...
        <Box component="div" className={classes.title}>
          ...
          <Box display="flex" justifyContent="flex-end" m={1}>
            <Button style={{ background: '#757575', marginBottom: 8 }}
              ...
              <Link className={classes.ft} to='/Admin/AddPromo'>
                Tambah Data
            </Link>
            </Button>
          </Box>
              ...
              <Paper className={classes.boxitem}>
                <Grid container spacing={3}>
                  ...
                  <Box style={{ marginLeft: 10 }}>
                      <Typography style={{ marginTop: -50, color: "white", fontWeight: 'bold' }}>Promo :{card.name}</Typography>
                      &nbsp;
                    <Typography style={{ color: "white", fontWeight: 'bold' }}>Discount :({card.discount})</Typography>
                    </Box>
