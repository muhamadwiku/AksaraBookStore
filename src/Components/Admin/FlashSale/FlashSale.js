...
class Product extends Component {
  ...
  componentDidMount() {
    fetch("http://beli.herokuapp.com/product-flash")
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
        ...
        <Box component="div" className={classes.title}>
          ...
          <Box display="flex" justifyContent="flex-end" m={1}>
            <Button style={{ background: '#757575', marginBottom: 8 }}
              ...
              <Link className={classes.ft} to='/Admin/AddFlashSale'>
                Tambah Data
            </Link>
            </Button>
          </Box>
              ...
              <Paper className={classes.boxitem}>
                <Grid container spacing={3}>
                  ...
                  <Box style={{ marginLeft: 10 }}>
                      <Typography style={{ marginTop: -50, color: "white", fontWeight: 'bold' }}>Harga sebelum diskon : {card.price1}</Typography>
                      &nbsp;
                    <Typography style={{ color: "white", fontWeight: 'bold' }}>Harga setelah diskon : ({card.price})</Typography>
                    </Box>
