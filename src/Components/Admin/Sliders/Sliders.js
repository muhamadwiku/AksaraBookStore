...
class Sliders extends Component {
  ...
  componentDidMount() {
    fetch("http://beli.herokuapp.com/all-slider")
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
        ...
        <Box component="div" className={classes.title}>
          ...
          <Box display="flex" justifyContent="flex-end" m={1}>
            <Button style={{ background: '#757575', marginBottom: 8 }}
              ...
              <Link className={classes.ft} to='/Admin/AddSliders'>
                Tambah Data
            </Link>
            </Button>
          </Box>
                ...
                <Grid container spacing={3}>
                  ...
                  <Box style={{ marginLeft: 10 }}>
                      <Typography style={{ marginTop: -50, color: "white", fontWeight: 'bold' }}>
                        Promo : {card.description}</Typography>
                      &nbsp;
                    <Typography style={{ color: "white", fontWeight: 'bold' }}>
                        Kode <a style={{ marginLeft: 10 }}>: ({card.code})</a></Typography>
                    </Box>
